        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="operate-search">
                    <a href="${ROOT}/admin/iretail/storeLevel/new.html" class="button button-blue"><i class="icon icon11"></i>新增</a>
                </div>
                    ...
                    <table class="table table-hover mytable">
                        ...
                        <c:forEach items="${pagelist.content}" var="item">
                            <tr>
                                <td>${item.id}</td>
                                <td>${item.name}</td>
                                <td>${item.decorateLevel}</td>
                                <td>${item.supportMoney}</td>
                                <td>${item.targetVolume}</td>
                                <td>${item.integralVolume}</td>
                                 <td><fmt:formatDate value="${item.createtime}" type="both" /></td>
                                <td><a href="${ROOT}/admin/iretail/storeLevel/edit-${item.id}" class="operate operate1"><i></i>编辑</a><a href="javascript:void(0);" onclick="deleteArea('${item.id}','${item.name }')" class="operate operate2"><i></i>删除</a></td>
                            </tr>
                        </c:forEach>
                        </tbody>
                    </table>
...
function deleteArea(id,name){
	if(confirm("确定要删除"+name+"吗？")){
		 location.href = "${ROOT}/admin/iretail/storeLevel/delete?id="+id;
	};
}
</script>
</body>
