	    ...
	    <div class="content"  align="center">
	      	 <form id="myFrom" class="form-horizontal"  action="${ROOT}/iretail/project/flow"  method="post">
	      		 <table class="maintable table table-hover"  style="width: 55%;">
		      		 <tr><td colspan="5"><span class="project_title">路演申请查看</span></td></tr>
		    		 <tr>
		     		 ...
		     		 <tr>
		     		 	...
		     		 	<td colspan="4" >
		     		 		...
		     		 		<c:forEach items="${jsonArray}" var="item">
		     		 			<tr><td>卖场名称:</td><td>${item.卖场名称}</td></tr>
		     		 			<tr><td>场地位置:</td><td>${item.场地位置}</td></tr>
		     		 			<tr><td>场地面积:</td><td>${item.场地面积}</td></tr>
		     		 			<tr><td>活动期间零售产品销量:</td><td>${item.活动期间零售产品销量}</td></tr>
		     		 			<tr><td>场租费用:</td><td>${item.场租费用}</td></tr>
		     		 			<tr><td>搭建费用:</td><td>${item.搭建费用}</td></tr>
		     		 			<tr><td>人员费用:</td><td>${item.人员费用}</td></tr>
		     		 			<tr><td>情况说明:</td><td>${item.情况说明}</td></tr>
		     		 			<tr>
		     		 				<td>实景图:</td>
		     		 				<td>
		     		 					<img width="330" src="${item.实景图url}"/>
		     		 				</td>
		     		 			</tr>
		     		 		</c:forEach>
		     		 		</table>
